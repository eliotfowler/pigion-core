...
jQuery(document).ready(function () {
    jQuery('#navigation').localScroll({offset: -80});
    jQuery('#mobile-nav').localScroll({offset: -47});
    jQuery('.action').localScroll({offset: -80});
    jQuery(function () {
        var sections = jQuery('section');
        var navigation_links = jQuery('nav a');
        sections.waypoint({handler: function (direction) {
            var active_section;
    ...
    jQuery(window).scroll(function () {
        ...
        if (ss >= 610) {
            jQuery('.sidephone-small').animate({left: 12}, 1200, "easeOutCubic");
        }
        if (ss >= 910) {
            jQuery('.static-phone').animate({bottom: 0}, 1200, "easeOutCubic");
        }
    });
    jQuery(function (jQuery) {
        jQuery('body').on('click', '#subscribe', function () {
            ...
            }, 'url': 'form.php', 'cache': false, 'data': jQuery(this).parents("form").serialize()});
            return false;
        });
    });
    jQuery(document.body).notifications();
    jQuery("#top").click(function () {
        return jQuery("body,html").stop().animate({scrollTop: 0}, 800, "easeOutCubic"), !1;
    });
